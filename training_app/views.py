...
def student_has_course_access(user, course: Course) -> bool:
    """Student can access course content if they have an active purchase for a published package containing it,
    OR if the course is in a FREE published package (no purchase required)."""
    # Check paid purchase
    has_purchase = PackagePurchase.objects.filter(
        student=user,
        status="active",
    ...
    if has_purchase:
        return True
    # Check free packages – no purchase needed
    is_in_free_pkg = Package.objects.filter(
        is_published=True,
        is_free=True,
        courses=course,
    ).exists()
    return is_in_free_pkg
...
def student_quiz_submission_detail(request, submission_id: int):
    ...
    serializer = QuizSubmissionReviewSerializer(
        submission,
        context={"request": request, "answers_by_question": answers_by_question, "submission": submission},
    )
    return Response(serializer.data)
